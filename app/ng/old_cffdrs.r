...
daily_duff_moisture_code <- function(dmc_yda, temp, rh, prec, lat, mon, lat.adjust = TRUE) {
  ...
  ell02 <- c(
    7.9, 8.4, 8.9, 9.5, 9.9, 10.2, 10.1, 9.7, 9.1,
    8.6, 8.1, 7.8
  )
  ell03 <- c(
    10.1, 9.6, 9.1, 8.5, 8.1, 7.8, 7.9, 8.3, 8.9,
    9.4, 9.9, 10.2
  )
  ell04 <- c(
    11.5, 10.5, 9.2, 7.9, 6.8, 6.2, 6.5, 7.4, 8.7,
    10, 11.2, 11.8
  )
...
daily_drought_code <- function(dc_yda, temp, rh, prec, lat, mon, lat.adjust = TRUE) {
  ...
  fl02 <- c(
    6.4, 5, 2.4, 0.4, -1.6, -1.6, -1.6, -1.6, -1.6,
    0.9, 3.8, 5.8
  )
...
daily_fwi <- function(input, init = data.frame(
  ...
  if (substr(names(init), 1, 1)[1] == "x" | substr(
    names(init),
    1, 1
  )[1] == "v") {
    if (ncol(init) == 3) {
