...
# -----------------------------------------------------------------------------
# FWI25 Results (hourly) — Results Table Module with DST policy normalization,
# robust parsing, and built-in debug mode.
# -----------------------------------------------------------------------------
...
#'   date (Date, optional), sunrise/sunset (decimal or ISO)
#' @param tz_reactive reactive Olson tz fallback (e.g., "America/Edmonton")
#' @param ignore_dst_reactive reactive policy: TRUE => fixed standard offset, FALSE => civil (DST-aware).
...
mod_results_table_server <- function(id, tr, dt_i18n, results, tz_reactive,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    normalize_policy <- function(val) {
      # Accept logical/character/numeric; return TRUE (ignore DST / fixed offset) or FALSE (civil/DST-aware)
      tolower_chr <- function(x) {
        x <- as.character(x); x[is.na(x)] <- ""; tolower(trimws(x))
      }
      ...
      if (isTRUE(getOption("fwi.debug_times", FALSE)))
        message("FWI25: normalize_policy() fell back to civil for value: ", paste0(capture.output(str(val)), collapse=" "))
      FALSE
    }
    ...
    # For DATETIME-like columns:
    # If string has Z/offset → parse as UTC instant.
    # If string is naive → parse as civil local time (per-row Olson), then convert to UTC.
    ...
    # For TIMESTAMP column specifically:
    # If string has Z/offset → parse as UTC instant (same as above).
    # If string is naive → **interpret as UTC** (NOT civil local).
    ...
    std_off_hours <- function(tz) {
      if (exists("tz_standard_offset_hours", mode = "function")) {
        tz_standard_offset_hours(tz)  # from mod_utils.R if available
      } else {
    ...
    # Extended ISO (hyphenated date) with offset, per row
    format_iso_with_offset_per_row <- function(x, tz_vec) {
      out <- character(length(x)); ok <- !is.na(x); out[!ok] <- NA_character_
      if (any(ok)) {
    ...
    add_debug_cols <- function(df, policy, display_tz_vec, std_off, olson_vec,
      ...
      if (!isTRUE(getOption("fwi.debug_times", FALSE))) return(df)
      cbind(
        df,
    ...
    table_data <- reactive({
      ...
      if (is.character(ts_raw)) df$timestamp <- parse_timestamp_to_utc(ts_raw)  # <-- key change
      ...
      if ("sunrise" %in% names(df) && is.character(df$sunrise)) {
        df$sunrise_utc <- parse_timestamp_to_utc(df$sunrise)  # sunrise strings are in UTC in your pipeline
      }
      ...
      # Build ISO strings with offsets for the visible time columns
      ...
      # Append debug columns (optional)
      out <- add_debug_cols(
        out, policy = ignore_dst, display_tz_vec = display_tz_vec, std_off = std_off,
