# R/modules/mod_log.R
# Complete replacement (v2) with optional metrics display; removed aria-label attribute for safety
  ...
  moduleServer(id, function(input, output, session){
    ...
    output$log <- renderPrint({
      ...
      if (!is.null(metrics)){
        ...
        cat(sprintf("Shape: %6.1f ms | Model: %6.1f ms | FWI87: %6.1f ms\n",
                    m$t_shape_ms %||% NA_real_, m$t_model_ms %||% NA_real_, m$t_daily_ms %||% NA_real_))
      ...
      if (!"datetime" %in% names(wx)){
        if (all(c("year","month","day","hour") %in% names(wx))){
          wx[, datetime := lubridate::make_datetime(year = as.integer(year), month = as.integer(month),
