    ...
    tags$section(
      ...
      div(
        ...
        div(
          ...
          # Client handlers: send layout and trace metadata back to Shiny for robust proxy updates
          tags$script(HTML("
            Shiny.addCustomMessageHandler('getPlotlyLayout', function(message) {
              var plot = document.getElementById(message.id);
              if (plot && plot.layout) {
                Shiny.setInputValue(message.id + '_layout', plot.layout, {priority:'event'});
              }
            });
            Shiny.addCustomMessageHandler('getPlotlyTraces', function(message) {
              var plot = document.getElementById(message.id);
              if (plot && plot.data) {
                var meta = {indices:[], id:[], srcType:[], yaxis:[], showlegend:[]};
                for (var i=0;i<plot.data.length;i++){
                  var tr=plot.data[i];
                  meta.indices.push(i);
                  // customdata columns: [source_label, station_id]
                  meta.id.push(tr.customdata && tr.customdata[0] ? tr.customdata[0][1] : null);
                  meta.srcType.push((tr.line && tr.line.dash==='dash')?'fwi87':'fwi25');
                  meta.yaxis.push(tr.yaxis||'y');
                  meta.showlegend.push(!!tr.showlegend);
                }
                Shiny.setInputValue(message.id+'_traces', meta, {priority:'event'});
              }
            });
          "))
        )
...
mod_plot_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # Decide rendering mode based on row count (keeps your performance heuristic)
    trace_mode_for <- function(n_rows) {
      if (isTRUE(n_rows <= 20000)) "lines+markers" else "lines"
    }

    # Only return a marker list when mode includes markers; otherwise NULL
    ...
    build_annotations <- function(layout, facet_titles, lbl_time) {
      ...
      for (i in seq_along(facet_titles)) {
        xn <- xnames[i]
        yn <- ynames[i]
        if (is.null(layout[[xn]]$domain) || is.null(layout[[yn]]$domain)) next
        xd <- layout[[xn]]$domain
        yd <- layout[[yn]]$domain
      ...
      # Global x-label (single authoritative x-title)
      ann[[length(ann) + 1]] <- list(
        text = lbl_time,
        x = 0.5, xref = "paper",
        y = -0.12, yref = "paper",
    ...
    # Color palettes (Okabe–Ito with Tableau fallback; single-station override)
    make_palettes <- function(id_vals, i18n_or) {
      ...
      if (n_ids == 1L) {
        pal_fwi25 <- c("#56B4E9") # Blue
        pal_fwi87 <- c("#E69F00") # Orange
      } else {
    ...
    # Consolidated proxy updater (titles, annotations, y-titles, legend names, tooltips)
    update_plot_language <- function(layout, traces_meta) {
      ...
      # Relayout payload (global title + annotations + y-axis titles)
      ...
      for (i in seq_along(input$plot_y_multi)) {
        ...
        relayout_payload[[paste0("yaxis", if (i > 1) i, ".title.standoff")]] <- 10
      }

      # Restyle payload (legend names + complete hover templates)

      templates <- vapply(seq_along(traces_meta$indices), function(i) {
        fi <- axis_index(traces_meta$yaxis[i])
        vlab <- if (!is.na(fi) && fi >= 1L && fi <= length(facet_titles)) {
          facet_titles[fi]
        } else {
          i18n_or("variable", "Variable")
        }
        build_hover_template_compact(lbl_series, lbl_station, variable_label = vlab, y_digits = 3)
      }, character(1))
      ...
      # Prefer to update only legend-visible traces, but updating all is simpler and consistent
      px %>% plotly::plotlyProxyInvoke("restyle", list(name = names_vec), list(traces_meta$indices))
      px %>% plotly::plotlyProxyInvoke("restyle", list(hovertemplate = templates), list(traces_meta$indices))
    }
    ...
    # Data source availability & dropdown
    ...
    data_for_plot <- reactive({
      ...
      # Detect datetime column or construct from Y/M/D/h
    ...
    # Populate Y choices based on dataset
    ...
    observeEvent(input$plot_dataset,
      {
        ...
        prefs <- preferred_for(input$plot_dataset)
        ...
        updateSelectizeInput(
          ...
          choices = stats::setNames(
            raw_choices,
            vapply(raw_choices, function(v) label_for_col(v, type = "short"), character(1))
          ),
    ...
    # -------------------------
    # RenderPlotly (no language in cache; proxy handles language)
    # -------------------------
    output$plot_ts <- plotly::renderPlotly({
      ...
      # Keep only needed columns and melt to long
      ...
      # Localized variable labels & factor order
      ...
      # Series key (source × station); source labels localized at render time
      ...
      make_facet_fig <- function(f_label) {
        ...
        for (idv in id_vals) {
          ...
          if (NROW(dd)) {
            ...
            fig <- plotly::add_trace(
              ...
              name = nm,
              hovertemplate = tip,
              customdata = cbind(rep(lab_fwi25, nrow(dd)), rep(idv, nrow(dd))),
          ...
          for (idv in id_vals) {
            ...
            if (NROW(dd87)) {
              ...
              fig <- plotly::add_trace(
                ...
                name = nm,
                hovertemplate = tip,
                customdata = cbind(rep(lab_fwi87, nrow(dd87)), rep(idv, nrow(dd87))),
                line = list(color = col, width = 1.8, dash = "dash"),
                # marker intentionally omitted for lines-only traces
                legendgroup = k,
                showlegend = show_leg
              )
            }
        ...
        # Facet axes; global x label is handled by annotations (subplot uses titleX = FALSE)
        fig <- plotly::layout(
          fig,
          yaxis = list(title = f_label),
          xaxis = list(title = i18n_or("plot_time_x", "Time"), tickformat = "%Y-%m-%d")
        )
        fig
      }
      ...
      sp <- do.call(
        plotly::subplot,
        c(figs,
          nrows = nrows, shareX = F, shareY = isFALSE(input$facet_free_y),
      ...
      # Add facet titles + global x annotation now
      sp <- plotly::layout(
        sp,
        annotations = build_annotations(sp$x$layout %||% list(), facet_titles = facet_levels, lbl_time = i18n_or("plot_time_x", "Time")),
        margin = list(t = 80)
      )
    ...
    observeEvent(lang(),
      {
        # Ask the browser for the current layout domains and trace indices
        session$sendCustomMessage("getPlotlyLayout", list(id = "plot_ts"))
        session$sendCustomMessage("getPlotlyTraces", list(id = "plot_ts"))
      },
      ignoreInit = TRUE
    )
