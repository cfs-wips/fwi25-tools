  ...
  moduleServer(id, function(input, output, session) {
    ...
    std_off_hours <- function(tz) {
      if (exists("tz_standard_offset_hours", mode = "function")) {
        ...
        z <- format(ref, "%z"); sgn <- ifelse(substr(z,1,1)=="-", -1, 1)
    ...
    parse_z_to_hours_fallback <- function(z) {
      z <- as.character(z); sgn <- ifelse(substr(z,1,1)=="-", -1, 1)
      hh <- suppressWarnings(as.integer(substr(z,2,3)))
      mm <- suppressWarnings(as.integer(substr(z,4,5)))
      sgn * (hh + mm/60)
    }
    ...
    make_palettes <- function(id_vals, i18n_or) {
      ...
      if (n_ids == 1L) { pal_fwi25 <- c("#56B4E9"); pal_fwi87 <- c("#E69F00") } else {
        ...
        to_fwi87 <- function(hex, desaturate = 0.20, lighten = 0.18) {
          rv <- grDevices::col2rgb(hex) / 255; gray <- mean(rv)
          rv <- rv + (gray - rv) * desaturate; rv <- pmin(rv + lighten, 1)
    ...
    output$plot_y_multi_label <- renderUI({ label_with_help(i18n_or("plot_vars","Select variables to plot"), i18n_or("tt_plot_vars","Choose one or more variables to visualize.")) })
    observe({
    ...
    data_for_plot <- reactive({
      ...
      df <- if (identical(input$plot_dataset, "inputs")) {
        si <- shaped_input(); validate(need(!is.null(si), i18n_or("err_upload_and_map_first","Upload and map your file first.")))
    ...
    output$plot_ts <- plotly::renderPlotly({
      df <- data_for_plot(); dt_col <- attr(df, "dt_col"); req(length(input$plot_y_multi) >= 1)
      yvars <- unique(input$plot_y_multi)
      keep_cols <- unique(c(dt_col, yvars, "id")); keep_cols <- intersect(keep_cols, names(df))
      ...
      if (identical(input$plot_dataset, "results")) {
        d87 <- df87(); if (!is.null(d87) && NROW(d87)) {
      ...
      pals <- make_palettes(id_vals, i18n_or); colour_map <- pals$map
      
      # Compute x_plot honoring DST policy
      tz_use <- tz_reactive() %||% "UTC"; use_std <- normalize_policy(ignore_dst_reactive())
      ...
      make_facet_fig <- function(f_label) {
        ...
        fig <- plotly::layout(
          ...
          xaxis = list(title = lbl_time, tickformat = "%Y-%m-%d")
        )
        fig
      }
      ...
      sp <- do.call(
        plotly::subplot,
        c(figs, nrows = nrows, shareX = FALSE, shareY = isFALSE(input$facet_free_y), titleX = FALSE, titleY = TRUE, margin = 0.03)
      )
      ...
      sp <- plotly::layout(
        sp,
        annotations = build_annotations(sp$x$layout %||% list(), facet_titles = facet_levels, lbl_time = lbl_time),
        margin = list(t = 80)
      )
