# Refactored mod_prepare.R with make_raw_like and convert_daily_to_hourly added
# Key changes:
# - Removed redundant TZ handling and hardcoded fallbacks.
# - Integrated tz$tz_use() and tz$tz_offset_policy() consistently.
# - Compute offset only if policy == "std"; otherwise use full TZ string.
# - Provenance includes tz_string and offset_policy.
# - Added make_raw_like() to ensure numeric columns and timestamp.
# - Reintroduced convert_daily_to_hourly() wrapper for daily→hourly conversion.
  ...
  moduleServer(id, function(input, output, session) {
    ...
    detect_resolution <- function(df, tz_string) {
      ...
      if (nzchar(dt_col) && dt_col %in% cols) {
        ...
        if (!inherits(dt$timestamp, "POSIXt")) {
          dt[, timestamp := suppressWarnings(as.POSIXct(timestamp, tz = tz_string, tryFormats = c(
    ...
    observeEvent(list(raw_file(), mapping_ready(), tz$tz_use(), tz$tz_offset_policy(), diurnal_method_reactive()), ignoreInit = FALSE, {
      ...
      if (det$kind == "daily") {
        ...
        res <- try(convert_daily_to_hourly(src, tz_string, diurnal_method = dia, offset_policy = offset_policy, skip_invalid = skip_invalid), silent = FALSE)
        if (inherits(res, "try-error") || is.null(res$hourly) || !nrow(res$hourly)) {
          ...
          out <- data.frame()
          attr(out, "provenance") <- list(source = "daily", conversion = "daily→hourly failed", tz = tz_string, offset_policy = offset_policy, offset_hours = tz_off, prepared_at = Sys.time())
          attr(out, "prep_log") <- logs
          raw_like_rv(out)
      ...
      out <- data.frame()
      attr(out, "provenance") <- list(source = "unknown", conversion = "n/a", tz = tz_string, offset_policy = offset_policy, offset_hours = tz_off, prepared_at = Sys.time())
      attr(out, "prep_log") <- logs
      raw_like_rv(out)
      meta_rv(list(kind = "unknown", converted = NA, tz = tz_string, log = logs))
    })

    list(raw_file = reactive(raw_like_rv()), prep_meta = reactive(meta_rv()))
  })
}
