...
# Existing behavior preserved; this adds src_daily for FWI87 usage downstream.
...
mod_prepare_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # external helpers you already source
    ...
    convert_daily_to_hourly <- function(df, tz_string, diurnal_method, offset_policy, skip_invalid) {
      ...
      mm <- as.data.frame(mm); names(mm) <- tolower(names(mm))
    ...
    # ---- resolution detection (unchanged) ----
    ...
    # ---- module state -----------------------------------------------------
    raw_like_rv   <- reactiveVal(NULL)   # hourly output (passthrough or converted)
    daily_src_rv  <- reactiveVal(NULL)   # NEW: original daily source when upload is daily
    meta_rv       <- reactiveVal(list(kind = NA, converted = FALSE, log = character()))
    last_kind_rv  <- reactiveVal(NULL)
    
    # ---- main observer: decides what to emit -----------------------------
    observeEvent(
      list(raw_file(), mapping_ready(), tz$tz_use(), tz$tz_offset_policy(), diurnal_method_reactive()),
      ignoreInit = FALSE,
      {
        src <- raw_file()
        ...
        # Case: hourly passthrough
        if (det$kind == "hourly") {
          out <- src
          attr(out, "provenance") <- list(
            source = "hourly", conversion = "passthrough",
            tz = tz_string, offset_policy = offset_policy, offset_hours = tz_off,
            prepared_at = Sys.time()
          )
          attr(out, "prep_log") <- logs
          
          raw_like_rv(out)
          daily_src_rv(NULL)  # NEW: clear daily source
          meta_rv(list(kind = "hourly", converted = FALSE, tz = tz_string, log = logs))
          return(invisible())
        }
        
        # Case: daily -> convert to hourly and also expose original daily
        if (det$kind == "daily") {
          logs <- c(
            logs,
          ...
          # even if conversion fails, still expose original daily (src_daily)
          daily_src <- src
          attr(daily_src, "provenance") <- list(
            source = "daily", conversion = "source_uploaded",
            tz = tz_string, offset_policy = offset_policy, offset_hours = tz_off,
            prepared_at = Sys.time()
          )
          attr(daily_src, "prep_log") <- logs
          daily_src_rv(daily_src)  # NEW: emit original daily
          ...
          if (inherits(res, "try-error") || is.null(res$hourly) || !nrow(res$hourly)) {
            ...
            out <- data.frame()
            attr(out, "provenance") <- list(
              source = "daily", conversion = "daily→hourly failed",
              tz = tz_string, offset_policy = offset_policy, offset_hours = tz_off,
              prepared_at = Sys.time()
            )
            attr(out, "prep_log") <- logs
          ...
          attr(raw_like, "provenance") <- list(
            source = "daily", conversion = paste0("daily→hourly(", dia, ")"),
            tz = tz_string, offset_policy = offset_policy, offset_hours = tz_off,
            prepared_at = Sys.time()
          )
          attr(raw_like, "prep_log") <- logs
          
          raw_like_rv(raw_like)                  # hourly for downstream FWI25
          meta_rv(list(kind = "daily", converted = TRUE, tz = tz_string, log = logs))
          
          emit_toast(sprintf(
            "Converted %s days to %s hourly \n using daily min/max and Beck & Trevitt.",
            nrow(src), nrow(raw_like)
          ), "message", 5)
        ...
        # Case: unknown
        logs <- c(logs, "[Prepare][ERROR] Could not classify input as hourly or daily.")
        out <- data.frame()
        attr(out, "provenance") <- list(
          source = "unknown", conversion = "n/a",
          tz = tz_string, offset_policy = offset_policy, offset_hours = tz_off,
          prepared_at = Sys.time()
        )
        attr(out, "prep_log") <- logs
    ...
    list(
      raw_uploaded = reactive(raw_file()),       # original upload (hourly or daily)
      hourly_file  = reactive(raw_like_rv()), # hourly (passthrough or converted)
      src_daily    = reactive(daily_src_rv()), # original daily if upload was daily
