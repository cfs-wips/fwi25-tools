...
mod_engine_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(run_click(), {
      ...
      # Invalidate reactives immediately (no defer)
    ...
    shaped_input_core <- reactive({
      ...
      if (!is.null(dt_col)) {
        ...
        validate(need(!is.null(dt_local) && !all(is.na(dt_local)), "Could not parse your Date + Time columns."))
      } else {
    ...
    daily_fwi_core <- reactive({
      ...
      if (!("id" %in% names(d87))) {
        # If cffdrs::fwi didn't echo id, try safe recovery by date (if unambiguous)
